...
# Review finding I6 (2026-09-10) put a 12-character floor on registration.
# These were "pw12345", which is 7 — a constant rather than eleven repeats
# so the next policy change is one edit, the way test_auth_hardening.py
# already does it.
...
async def test_login_sets_httponly_refresh_cookie():
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "refresh-a@voiceagent-test.com", "password": PASSWORD})
        resp = await client.post("/auth/login", json={"email": "refresh-a@voiceagent-test.com", "password": PASSWORD})
...
async def test_refresh_issues_a_new_access_token():
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "refresh-b@voiceagent-test.com", "password": PASSWORD})
        await client.post("/auth/login", json={"email": "refresh-b@voiceagent-test.com", "password": PASSWORD})
...
async def test_refresh_token_is_rotated_and_old_one_stops_working():
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "refresh-c@voiceagent-test.com", "password": PASSWORD})
        await client.post("/auth/login", json={"email": "refresh-c@voiceagent-test.com", "password": PASSWORD})
...
async def test_logout_revokes_the_refresh_token():
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "logout-a@voiceagent-test.com", "password": PASSWORD})
        await client.post("/auth/login", json={"email": "logout-a@voiceagent-test.com", "password": PASSWORD})
...
async def test_a_refresh_token_cannot_be_used_as_an_access_token():
    ...
    of separating them."""
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "typeconfusion@voiceagent-test.com", "password": PASSWORD})
        await client.post("/auth/login", json={"email": "typeconfusion@voiceagent-test.com", "password": PASSWORD})
...
async def test_login_is_rate_limited():
    async with await _fresh_client() as client:
        await client.post("/auth/register", json={"email": "ratelimit@voiceagent-test.com", "password": PASSWORD})
